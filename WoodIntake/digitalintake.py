...
class DigitalIntake:
    ...
    def get_data_api(self):
        ...
        # api-endpoint
        URL = os.environ.get("DATABASE_URL") + "/wood"
    ...
    def post_call_wood(self, row):
        
        url = os.environ.get("DATABASE_URL")

        # print(row)
        json_row = json.dumps(row)
        st.write(json.dumps(row))
    ...
    def delete_requirements_from_client(self, part_index = 1000, delete_all = False):
        ...
        if delete_all == True:
            ...
            for index in pd.DataFrame(response.json())['id']:
                response = requests.delete(url=delete_endpoint + '/'+ str(index), headers={"Content-Type": "application/json"})
                print(response.json())
        else:
...
def main():
    ...
    if sidebar == 'Database':
        ...
        form_finding = form_finding_page(DigIn)
