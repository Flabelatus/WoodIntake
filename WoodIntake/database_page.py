...
class database_page:
    ...
    def __init__(self, Digital_intake_class):
        ...
        st.divider()
        st.header("Digital Intake Results from the Robot Lab")
        dataset = DigIn.data
        # style = 'API'
        
        dataset_tab1, dataset_tab2, dataset_add_json, dataset_tab3 = st.tabs(["Current database", 
            "Add elements to the dataset", "Add elements with a JSON","Login"])
        ...
        with dataset_add_json:
            if not authentication_status:
                st.write("You have to login to add elements to the database")
            else:
                new_row = st.text_area("JSON row with an example", 
                    value=json.dumps({
                "color": "150,143,116",
                "density": 626.0,
                "height": 20.0,
                ...
                "storage_location": "B5",
                "timestamp": "2023-10-10 10:40-19",
                "type": "hardwood",
                "production_phase": "Received",
                "verified_measurements": True,
                "weight": 3350.0,
                "width": 164.0}))

             # Try to load the JSON string
                json_data = json.loads(new_row)
                st.write("Successfully loaded JSON:")
                st.json(json_data)


            if st.button('Add wood piece to data', key = 'JSON_add_button'):
                
                response = DigIn.post_call_wood(json_data)
                st.write(response)
        ...
        with dataset_tab2:
            if not authentication_status:
                ...
                with col1:
                    ...
                    production_phase_radio = st.radio("Which production phase?", ["Received", "Planed", "Sawed", "Drilled", "Finished"], 
                        key = 'production_phase_radio')
                    original_piece_input = st.text_input("Is there a known original piece?", value = "1")
                    verified_measurements_input = st.radio("Verified measurements?",[True, False])
            ...
            if st.button('Add wood piece to data', key = 'custom_add_button'):
                DigIn.post_call_wood(new_row)
                DigIn.wood_list.append(new_row)
    ...
    def database_tabs(self, dataset, DigIn, Derako_row):
         # Function to handle different tabs in the Streamlit UI
        ...
        st.header("Extra information on the database")
